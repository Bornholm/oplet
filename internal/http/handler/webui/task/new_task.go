...
func (h *Handler) handleNewTaskSubmission(w http.ResponseWriter, r *http.Request, storeTask *store.Task, taskDef *task.Definition) {
	...
	req, err := h.createExecutionRequest(ctx, storeTask, taskForm.Values, taskForm.Files, taskDef, taskExecution.ID)
	if err != nil {
...
func (h *Handler) createExecutionRequest(ctx context.Context, storeTask *store.Task, values map[string]string, files map[string][]*multipart.FileHeader, taskDef *task.Definition, executionID uint) (task.ExecutionRequest, error) {
	req := task.ExecutionRequest{
		ImageRef:    storeTask.ImageRef,
	...
	for _, config := range storeTask.Configurations {
		req.Environment[config.Name] = config.Value
	}
