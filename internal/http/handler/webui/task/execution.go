...
func (h *Handler) getExecutionProgressPage(w http.ResponseWriter, r *http.Request) {
...
func (h *Handler) getExecutionLogs(w http.ResponseWriter, r *http.Request) {
	...
	logsComponent := component.LogEntries(logs)
...
func (h *Handler) fillTaskExecutionHistoryPageViewModel(r *http.Request, taskID uint) (*component.TaskExecutionHistoryPageVModel, error) {
	...
	// Get executions
	executionRepo := execution.NewRepository(h.store)
	executions, err := executionRepo.GetByTaskID(ctx, taskID, 50, 0) // Limit to 50 for now
	if err != nil {
		return nil, errors.WithStack(err)
	}

	// Get stats
	stats, err := executionRepo.GetExecutionStats(ctx, taskID)
	if err != nil {
		return nil, errors.WithStack(err)
	}
...
func (h *Handler) fillGlobalExecutionHistoryPageViewModel(r *http.Request) (*component.GlobalExecutionHistoryPageVModel, error) {
	...
	// Get executions
	executionRepo := execution.NewRepository(h.store)
	executions, err := executionRepo.SearchExecutions(ctx, filters, 50, 0) // Limit to 50 for now
	if err != nil {
		return nil, errors.WithStack(err)
	}
...
func (h *Handler) canAccessExecution(ctx context.Context, executionID uint) bool {
	// For now, allow all authenticated users to access executions
	// In a real implementation, you'd check if the user owns the execution or is an admin
	return true
}
