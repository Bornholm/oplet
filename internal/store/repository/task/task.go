...
func (r *Repository) GetByID(ctx context.Context, id uint) (*store.Task, error) {
	...
	err := r.store.WithDatabase(ctx, func(ctx context.Context, db *gorm.DB) error {
		if err := db.Preload(clause.Associations).First(&task, id).Error; err != nil {
			return errors.WithStack(err)
		}
		return nil
	})
...
func (r *Repository) GetByImageRef(ctx context.Context, imageRef string) (*store.Task, error) {
	var task store.Task
	err := r.store.WithDatabase(ctx, func(ctx context.Context, db *gorm.DB) error {
		if err := db.Preload(clause.Associations).Where("image_ref = ?", imageRef).First(&task).Error; err != nil {
			return errors.WithStack(err)
		}
		return nil
	})
...
func (r *Repository) Update(ctx context.Context, task *store.Task) error {
	return r.store.WithTx(ctx, func(ctx context.Context, db *gorm.DB) error {
...
func (r *Repository) UpdateConfiguration(ctx context.Context, taskID uint, values map[string]string) error {
	return r.store.WithTx(ctx, func(ctx context.Context, db *gorm.DB) error {
		var task store.Task
		...
		if err := db.Unscoped().Model(&task).Association("Configurations").Clear(); err != nil {
			return errors.WithStack(err)
		}
		...
		for name, value := range values {
			...
			configurations = append(configurations, tc)
		}

		if err := db.Model(&task).Association("Configurations").Replace(configurations...); err != nil {
			return errors.WithStack(err)
		}
...
func (r *Repository) Delete(ctx context.Context, id uint) error {
	return r.store.WithDatabase(ctx, func(ctx context.Context, db *gorm.DB) error {
		if err := db.Unscoped().Delete(&store.Task{}, id).Error; err != nil {
			return errors.WithStack(err)
		}
		return nil
	})
