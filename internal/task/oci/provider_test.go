...
func TestProvider_EndToEnd_Simulation(t *testing.T) {
	...
	mockLabels := map[string]string{
		...
		"io.oplet.task.inputs.output_format.type":        "text",
		...
		"io.oplet.task.config.max_rows.type":        "number",
	...
	if inputFile == nil {
		...
		if inputFile.Type != task.TypeFile {
			t.Errorf("input_file type: expected %v, got %v", task.TypeFile, inputFile.Type)
		}
	}
