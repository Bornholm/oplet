...
func (e *DockerExecutor) Execute(ctx context.Context, req task.ExecutionRequest) error {
	...
	go func() {
		...
		// Create container
		containerID, err := e.createContainer(ctx, runID, req)
		if err != nil {
			execution.State = task.ExecutionStateFailed
			execution.Error = &task.ExecutionError{
		...
		case <-ctx.Done():
			...
			defer cancel()
			if err := e.client.ContainerStop(shutdownCtx, containerID, container.StopOptions{}); err != nil {
				slog.ErrorContext(shutdownCtx, "could not stop container", slog.String("containerID", containerID))
			}
			execution.State = task.ExecutionStateKilled
			execution.Error = &task.ExecutionError{
				Type:        task.ErrorTypeDockerDaemonError,
...
func (e *DockerExecutor) prepareCacheVolumes(ctx context.Context, imageRef string) ([]string, error) {
	inspect, err := e.client.ImageInspect(ctx, imageRef)
	if err != nil {
		return nil, errors.Wrap(err, "failed to inspect image for volumes")
	}

	var binds []string

	for containerPath := range inspect.Config.Volumes {
		// Generate a named volume for this path
		volName := generateCacheVolumeName(imageRef, containerPath)

		// Docker Bind Format: "volume_name:container_path"
		bindEntry := fmt.Sprintf("%s:%s", volName, containerPath)
		binds = append(binds, bindEntry)

		e.logger.DebugContext(ctx, "auto-configured cache volume",
			"path", containerPath,
			"volume", volName,
		)
	}
...
func (e *DockerExecutor) createContainer(ctx context.Context, runID string, req task.ExecutionRequest) (string, error) {
	...
	cacheBinds, err := e.prepareCacheVolumes(ctx, req.ImageRef)
	if err != nil {
		return "", errors.Wrap(err, "could not setup cache volumes")
	}

	env = append(env, fmt.Sprintf("OPLET_RUN_ID=%s", runID))
	...
	// Create container
	resp, err := e.client.ContainerCreate(ctx, config, hostConfig, nil, nil, fmt.Sprintf("oplet-task-%s", runID))
	if err != nil {
...
func (e *DockerExecutor) remove(ctx context.Context, containerID string) error {
	...
	for _, m := range inspect.HostConfig.Mounts {
		...
		if err := e.client.VolumeRemove(ctx, m.Source, true); err != nil {
			return &task.ExecutionError{
				Type:        task.ErrorTypeDockerDaemonError,
				Message:     fmt.Sprintf("failed to remove volume '%s'", m.Source),
				ContainerID: containerID,
				Cause:       errors.WithStack(err),
			}
		}

		e.logger.Debug("volume removed", "volume_id", m.Source)
...
// generateCacheVolumeName creates a consistent name for the volume.
// Format: oplet-cache-<sanitized_image_name>-<hash_of_mount_path>
func generateCacheVolumeName(imageName, mountPath string) string {
	// 1. Sanitize image name (remove registry, tag, special chars)
	// Example: "ghcr.io/my-team/scrapper:latest" -> "scrapper"
	cleanImage := imageName
	if lastSlash := strings.LastIndex(cleanImage, "/"); lastSlash != -1 {
		cleanImage = cleanImage[lastSlash+1:]
	}
	if tagIndex := strings.LastIndex(cleanImage, ":"); tagIndex != -1 {
		cleanImage = cleanImage[:tagIndex]
	}
	cleanImage = strings.Map(func(r rune) rune {
	...
	// 2. Hash the path to ensure uniqueness and safe characters (handling deeply nested paths)
	h := sha256.New()
	h.Write([]byte(mountPath))
	pathHash := fmt.Sprintf("%x", h.Sum(nil))[:8]

	return fmt.Sprintf("oplet-cache-%s-%s", cleanImage, pathHash)
}
