...
func (p *Parser) parseInputLabels(labels map[string]string, parsed *ParsedLabels) error {
	...
	for name, props := range configGroups {
		...
		if configLabels.Label == "" {
			configLabels.Label = name
		}
...
func (p *Parser) buildInputLabels(props map[string]string) (InputLabels, error) {
	inputLabels := InputLabels{
		...
		Tags:        strings.Split(props[PropertyTags], " "),
	}
	...
	if !p.isValidType(inputLabels.Type) {
		return inputLabels, errors.Errorf("invalid type '%s', must be 'text', 'number', 'file', 'secret' or 'boolean'", inputLabels.Type)
	}

	return inputLabels, nil
}

// isValidType checks if the value type is valid
func (p *Parser) isValidType(inputType string) bool {
	return inputType == string(task.TypeText) ||
		inputType == string(task.TypeNumber) ||
		inputType == string(task.TypeFile) ||
		inputType == string(task.TypeSecret) ||
		inputType == string(task.TypeBoolean)
}
...
func (p *Parser) convertToTaskInput(name string, inputLabels InputLabels) (*task.Input, error) {
	...
	return &task.Input{
		...
		Type:        task.Type(inputLabels.Type),
