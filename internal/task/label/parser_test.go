...
func TestParser_ParseLabels(t *testing.T) {
	parser := NewParser()
	...
	tests := []struct {
	...
	}{
		{
			...
			labels: map[string]string{
				...
				"io.oplet.task.config.max_memory.type":        "number",
		...
		},
		{
			...
			labels: map[string]string{
				...
				"io.oplet.task.inputs.input1.type": "text",
			},
			expected: &ParsedLabels{
				Meta: MetaLabels{
					Name: "Simple Task",
		...
		},
		{
			...
			labels: map[string]string{
				...
				"io.oplet.task.inputs.input1.label": "invalid",
			},
			expectError: true,
		},
		{
...
func TestParser_BuildTaskDefinition(t *testing.T) {
	parser := NewParser()
	...
	}{
		{
			...
			validate: func(t *testing.T, def *task.Definition) {
				...
				if inputFile == nil {
					...
					if inputFile.Type != task.TypeFile {
						t.Errorf("input_file.InputType: expected %v, got %v", task.TypeFile, inputFile.Type)
					}
